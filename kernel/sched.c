...
// 8253定时器 时钟周期（clock cycle）的频率 -- 晶体振荡器在1秒时间内产生的时钟脉冲个数
// HZ = 100,  也就是 1秒钟内 产生 100个时间片， 一次滴答（时间片）是 10ms，
// 把 LATCH 值注入 8253定时器, 定时器倒数计时，减为0时，就产生一次时钟中断。
// 在IRQ0上产生一次时钟中断，也即一次时钟滴答
